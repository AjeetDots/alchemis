...
/**
 * Defines the app_report_Report12 class.
 ...
 * @package Alchemis
 */
class app_report_Report12 extends FPDF
{
...
//        $this->params['client_id']                     = $client_id;
        ...
        $this->globalSectorDisciplineEffectivenessSection();
	...
	public function Header()
	{
		...
		$this->Cell(50, 5, 'Report ID 12', 0, 0, 'L', 0);
		$this->SetFont('Arial', 'B', 10);
		$this->Cell(90, 5, 'Sector Discipline Analysis Report', 0, 0, 'C', 0);
		$this->SetFont('Arial', '', 8);
    ...
    public function globalSectorDisciplineEffectivenessSection()
    {
        $currentSector = '';

        $this->AddPage();

        $rows = app_domain_ReportReader::getReport12GlobalSectorDisciplineAnalysis($this->params['start'], $this->params['end']);
        ...
        foreach ($rows as $row)
        {
            if ($currentSector != $row['sector']) {
                $this->SetFont('Arial', 'B', 10);
                $this->Ln();
                $this->Cell(80, 5, 'Sector: ', 1, 0, 'L', 0);
                $this->Cell(110, 5, $row['sector'], 1, 0, 'L', 0);
                $this->Ln();
