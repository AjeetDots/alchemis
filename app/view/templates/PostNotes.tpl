{include file="header2.tpl" title="Post Notes"}
...
//	alert('submitform(' + pressbutton + ')');
	document.add_post_note.task.value=pressbutton;
	
	try
	{
		document.add_post_note.onsubmit();
	}
	
	catch(e)
	{}
	
	document.add_post_note.submit();
}
...
function makeInPlaceEditor(note_id)
{
	...
	var in_place_editor = new Ajax.InPlaceEditor('edit_note_' + note_id , '', {rows:15, cols:40, externalControl: 'img_edit_note_' + note_id, ill_cmd: 'AjaxPost', ill_cmd_action: 'update_note', ill_item_id: note_id, ill_field: 'note'});
...
{/literal}
parent.$('post_note_count').innerHTML={$notes|@count};
</script>

<div id="div_post_notes_screen" class="module_content" style="border: solid 1px #ccc; padding: 2px; overflow: auto;">
	...
	<div id="div_add_new_note" style="display: none">
		<form action="index.php?cmd=PostNotes" method="post" name="add_post_note" autocomplete="off">
		...
		<table style="width: 100%" class="default">
			...
			<tr>
				<td>
					...
					<span id="edit_note_{$result.id}">{$result.note}</span>
				</td>
			</tr>
			<tr>
				<td>
