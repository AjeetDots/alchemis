		...
		<tbody>
			{foreach name="result_loop" from=$search_results item=result}
				<tr id="tr_post_{$result.post_id}" style="vertical-align:top">
					...
					{*<td><a href="#" onclick="javascript:new Effect.toggle($('post_list_{$result.id}'), 'blind', {literal}{duration: 0.3}{/literal});return false; ">[Posts...]</a></td>*}
					<td>{$result.id}</td>
					<td>
						<span id="client_{$result.id}">{$result.name}</span>
						{if $result.telephone != ""}
						<br />
					...
					<td style="text-align: center; background-color: #F3F3F3">
						<a id="detailsBtn_{$result.id}" title="Edit" href="#" onclick="javascript:showPost({$result.id}, {$result.post_id});return false;"><img src="{$APP_URL}app/view/images/icons/database_table.png" alt="Details" title="Details" /></a>
					</td>
				</tr>
				<script language="JavaScript" type="text/javascript">
					var row = new Object;
					...
					{if $search_results|@count == 1}
						showPost({$result.id}, {$result.post_id});
					{/if}
