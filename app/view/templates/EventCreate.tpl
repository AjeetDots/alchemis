{include file="header2.tpl" title="Event Create"}

{if $success}

	<p>The event has been created.</p>
{*	<input type="button" id="btn_add_action" value="Add another action" onclick="javascript:document.location.href='index.php?cmd=ActionCreate'" />*}

	{*<p>Redirect to 'index.php?cmd=Calendar&nbm_id={$user_id}&date={$app_domain_Event_due_date}'</p>*}
	<p>Redirecting to calendar...</p>
	<script language="JavaScript" type="text/javascript">
		{*parent.location = 'index.php?cmd={$referrer}&date={$app_domain_Event_due_date}';*}
		parent.location = 'index.php?cmd=Calendar&nbm_id={$user_id}&date={$app_domain_Event_due_date}';
	</script>
{else}
...
function validate()
{
	...
	if ($F("app_domain_Event_due_date") != "")
	{
		// Make the selected date into a js format date & adjust for timezone
		var raw_date_string = $F("app_domain_Eventdue_date") + " " + $F("next_communication_time_Hour") + ":" + $F("next_communication_time_Minute");
...
{
	handleSelect('app_domain_Event_due_date', type, args, obj);
}

function handleSelectReminderDate(type, args, obj)
{
	handleSelect('app_domain_Event_reminder_date', type, args, obj);
}

// Handles input to the calendar from recall_date text field when the ... button is clicked
function updateDueDate() 
{ 
	Effect.toggle($('app_domain_Event_due_date_calendar_display'), 'blind', {duration: 0.3});
	var txtDate1 = $('app_domain_Event_due_date');
	
	if (txtDate1.value != '')
	{
		// Select the date typed in the field
		YAHOO.example.calendar.cal_app_domain_Event_due_date.select(txtDate1.value);
	}
	YAHOO.example.calendar.cal_app_domain_Event_due_date.render();
}

// Handles input to the calendar from recall_date text field when the ... button is clicked
function updateReminderDate() 
{ 
	Effect.toggle($('app_domain_Event_reminder_date_calendar_display'), 'blind', {duration: 0.3});
	var txtDate1 = $('app_domain_Event_reminder_date');
	
	if (txtDate1.value != '')
	{
		// Select the date typed in the field
		YAHOO.example.calendar.cal_app_domain_Event_reminder_date.select(txtDate1.value);
	}
	YAHOO.example.calendar.cal_app_domain_Event_reminder_date.render();
}

function init()
{
	// Due date
	YAHOO.example.calendar.cal_app_domain_Event_due_date = new YAHOO.widget.Calendar("cal_app_domain_Event_due_date", "div_cal_app_domain_Event_due_date");
	YAHOO.example.calendar.cal_app_domain_Event_due_date.cfg.setProperty("START_WEEKDAY", 1);
	YAHOO.example.calendar.cal_app_domain_Event_due_date.cfg.setProperty("MDY_DAY_POSITION", 1);
	YAHOO.example.calendar.cal_app_domain_Event_due_date.cfg.setProperty("MDY_MONTH_POSITION", 2);
	YAHOO.example.calendar.cal_app_domain_Event_due_date.cfg.setProperty("MDY_YEAR_POSITION", 3);
	YAHOO.example.calendar.cal_app_domain_Event_due_date.cfg.setProperty("MD_DAY_POSITION", 1);
	YAHOO.example.calendar.cal_app_domain_Event_due_date.cfg.setProperty("MD_MONTH_POSITION", 2);
	YAHOO.example.calendar.cal_app_domain_Event_due_date.selectEvent.subscribe(handleSelectDueDate, YAHOO.example.calendar.cal_app_domain_Event_due_date, true);
	YAHOO.example.calendar.cal_app_domain_Event_due_date.render();
	
	// Reminder date
	YAHOO.example.calendar.cal_app_domain_Event_reminder_date = new YAHOO.widget.Calendar("cal_app_domain_Event_reminder_date", "div_cal_app_domain_Event_reminder_date");
	YAHOO.example.calendar.cal_app_domain_Event_reminder_date.cfg.setProperty("START_WEEKDAY", 1);
	YAHOO.example.calendar.cal_app_domain_Event_reminder_date.cfg.setProperty("MDY_DAY_POSITION", 1);
	YAHOO.example.calendar.cal_app_domain_Event_reminder_date.cfg.setProperty("MDY_MONTH_POSITION", 2);
	YAHOO.example.calendar.cal_app_domain_Event_reminder_date.cfg.setProperty("MDY_YEAR_POSITION", 3);
	YAHOO.example.calendar.cal_app_domain_Event_reminder_date.cfg.setProperty("MD_DAY_POSITION", 1);
	YAHOO.example.calendar.cal_app_domain_Event_reminder_date.cfg.setProperty("MD_MONTH_POSITION", 2);
	YAHOO.example.calendar.cal_app_domain_Event_reminder_date.selectEvent.subscribe(handleSelectReminderDate, YAHOO.example.calendar.cal_app_domain_Event_reminder_date, true);
	YAHOO.example.calendar.cal_app_domain_Event_reminder_date.render();
}
		...
		<fieldset class="adminform">
			...
			<table>
				...
				<tr>
					...
					<td>
						<select name="app_domain_Event_type_id" id="app_domain_Event_type_id">
							{html_options options=$event_types selected=$app_domain_Event_type_id}
						</select>
					</td>
				</tr>
				<tr>
					...
					<td>
						...
						<div id="app_domain_Event_due_date_calendar_display" style="display: none">
							<div id="div_cal_app_domain_Event_due_date"></div> 
						</div>
					</td>
				</tr>
				<tr>
					...
					<td>
						<select name="app_domain_Event_day_part" id="app_domain_Event_day_part">
...
{*				<tr>
					<td>
						{if $app_domain_Event_due_date}
							{assign var=due_date_time value=$app_domain_Event_due_date|date_format:'%H:%M'}
						{else}
					...
					<tr>
						...
						<td>
							...
							<div id="app_domain_Event_reminder_date_calendar_display" style="display: none">
								<div id="div_cal_app_domain_Event_reminder_date"></div> 
							</div>
						</td>
					</tr>
					<tr>
