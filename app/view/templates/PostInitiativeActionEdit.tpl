{include file="header2.tpl" title="Post Initiative Action Edit"}
...
//	alert('submitform(' + pressbutton + ')');
	...
	// set any disabled fields to enabled if we need to submit then - eg action type id is disabled if its for an existing action
	$('app_domain_Action_type_id').enable();
	try
	{
...
{/literal}
	...
	<form action="index.php?cmd=PostInitiativeActionEdit" method="post" name="adminForm" autocomplete="off">
		...
		<fieldset class="adminform">
			{*<legend>{if $action_id}Edit{else}Add{/if} action for {$post->getContactName()}</legend>
			(for {$initiative_name})
			<br />
			<br />*}
			...
			<table>
				...
				<tr>
					...
					<td>
						<select style="width: 100%" id="app_domain_Action_type_id" name="app_domain_Action_type_id"{if $action_id} disabled{/if} />
							<option value="0">&ndash; Select &ndash;</option>
							{html_options options=$type_options selected=$app_domain_Action_type_id}
						</select>
					</td>
				</tr>
				
				<tr>
					<td style="width: 110px; vertical-align:top" {if $errors.app_domain_Action_notes} class="key_error" title="{$errors.app_domain_Action_notes->getTip()}"{else}class="key"{/if}>
				...
				<tr>
					...
					<td>
						<select style="width: 100%" id="app_domain_Action_communication_type_id" name="app_domain_Action_communication_type_id">
							<option value="0">&ndash; Select &ndash;</option>
							{html_options options=$communication_type_options selected=$communication_type_default}
							{*{html_options options=$communication_type_options selected=$app_domain_Action_communication_type_id}*}
						</select>
					</td>
				</tr>
				<tr>
					...
					<td>
						<select style="width: 100%" id="app_domain_Action_resources" name="app_domain_Action_resources[]" multiple="multiple" size="6">
							{html_options options=$resource_type_options selected=$app_domain_Action_resources}
						</select>
					</td>
				</tr>
				<tr>
					...
					<td>
						<input type="checkbox" id="app_domain_Action_actioned_by_client" name="app_domain_Action_actioned_by_client" {if $app_domain_Action_actioned_by_client} checked{/if}/>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td>
						<input type="checkbox" id="chk_display_reminder" name="chk_display_reminder"{if $app_domain_Action_reminder_date} checked="checked"{/if} onchange="javascript: new Effect.toggle($('div_display_reminder'), 'blind', {literal}{duration: 0.3}{/literal});return false;" />
					</td>
				</tr>
