{include file="header.tpl" title="Campaign List"}
...
<fieldset class="adminform">
	...
	<table id="table1" class="adminlist"{* class="sortable" border="0" cellpadding="0" cellspacing="1" width="100%"*}>
		<thead>
			<tr>
				<th style="width: 3%">#</th>
				<th style="width: 47%; text-align: center">Campaign</th>
		...
		<tbody>
			...
			{assign var=client value=$campaign->getClient()}
			
			<tr>
				<td>{$smarty.foreach.cam.iteration}</td>
				<td style="text-align: center">{$campaign->getName()}</td>
				<td style="text-align: center">{$campaign->getRevision()}</td>
				<td style="text-align: center">{$campaign->getId()}</td>
				<td style="text-align: center">{*$client->getName()*}</td>
				<td style="text-align: center">{$campaign->getCreated()|date_format:"%A, %B %e, %Y"}</td>
				<td style="text-align: center; background-color: #F3F3F3">
					<div class="button2-left">
						<div class="page"><a id="viewBtn_{$campaign->getId()}" title="Details" href="index.php?cmd=CampaignView&amp;campaign_id={$campaign->getId()}">Details</a></div>
					</div>
					<div class="button2-left" style="display: none">
						<div class="page"><a id="cancelBtn_{$campaign->getId()}" title="Cancel" onclick="{*javascript:cancelEditable({$campaign->getId()});*}">Cancel</a></div>
					</div>
					<div class="button2-left">
