	...
	<table id="table1" class="adminlist sortable" cellspacing="1">
		...
		<thead>
			...
			<tr>
				...
				<th style="width: 24%; text-align: left">Last Effective</th>
				...
				<tr id="{if $result.post_initiative_id != ""}tr_post_initiative_{$result.post_initiative_id}{elseif $result.post_id != ""}tr_post_{$result.post_id}{else}tr_{$result.id}{/if}" style="vertical-align:top">
					...
					<td>
						...
						{if $result.job_title}
							...
							{if $result.surname != "" || $result.first_name != ""}

			 					<br />

			 					<strong>{$result.title}&nbsp;{$result.first_name}&nbsp;{$result.surname}</strong>

			 				{/if}
			 				{if $result.telephone_1 != ''}
				 				<br />
				 				<br />
					...
					<td>
						...
						<br />
		 				<br />
						<span>
							<span style="float:left">{$result.client_name}:&nbsp;{$result.initiative_name}</span>
							{*<span style="float:right"><input id="chk_client_{$result.post_initiative_id}" name="chk_client_{$result.post_initiative_id}" type="checkbox" /></span>*}
						</span>	
					</td>

					<td>

						{$result.communication_date|date_format:"%d %B %Y"}
