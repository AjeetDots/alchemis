...
top.parent.scroll_positions[4] = "true";
...
function getColln () {
	var params = {};
	var frame6 = top.iframe_6.contentWindow;
	if(!frame6.iframe1) return;
	var contentWindow = frame6.iframe1.contentWindow;
	location.search.split('&').forEach(function (item) {
		var param = item.replace('#').split('=');
		params[param[0]] = param[1];
	});
	if(params.parent_id){
		return contentWindow.collns[params.parent_id];
	}
	return contentWindow.colln;
}

function updateCount (company_id) {
	
	if(typeof frame6 == 'undefined') return;
	var companies = frame6.iframe1.contentWindow.companies;
	if(!companies) return;

	var ele = jQuery('#company_count');
	var text = (companies.indexOf(company_id) + 1) + ' of ' + companies.length;
	ele.text(text);
