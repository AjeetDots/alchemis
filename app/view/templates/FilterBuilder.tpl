...
function doSaveFilter(id, load_filter)
{
	if (validate())
	{
		...
		if (id ==0)
		{
			if (load_filter)
			{
				getAjaxData("AjaxFilterBuilder", "", "save_as_and_load_filter", ill_params, "Saving...");
			}
			else
			{
				getAjaxData("AjaxFilterBuilder", "", "save_as_filter", ill_params, "Saving...");
			}
		}
		else
		{
			if (load_filter)
			{
				getAjaxData("AjaxFilterBuilder", "", "save_and_load_filter", ill_params, "Saving...")
			}
			else
			{
...
function saveFilter()
{
	doSaveFilter($F("id"), false);
}

function saveAndLoadFilter()
{
	doSaveFilter($F("id"), true);
}

function saveAsFilter()
{
	if ($F("filter_name") == $F("filter_name_orig")) 
	{
		alert("You must choose a name for this filter that is different from '" + $F("filter_name_orig") + "'.");
	}
	else
	{
		doSaveFilter(0, false);
	}
}

function saveAsAndLoadFilter()
{
	if ($F("filter_name") == $F("filter_name_orig")) 
	{
		alert("You must choose a name for this filter that is different from '" + $F("filter_name_orig") + "'.");
...
var printFilterWindowObjectReferencePopup;
...
if(typeof window.screenX == "number" && typeof window.innerWidth == "number")
	...
	windowLeft = window.screenX + window.innerWidth;
	windowTop = window.screenY + window.innerHeight;
	}
...
printFilterWindowObjectReferencePopup = window.open("index.php?cmd=FilterBuilderPrint&id=" + $F("id"), "printFilterPopup", "top=" + windowTop + ",left=" + windowLeft + ",width=" + windowWidth + ",height=" + windowHeight + ",,,,resizable,scrollbars,status");
printFilterWindowObjectReferencePopup.focus();
	...
	for (i = 1; i < data.length + 1; i++) 
	{
		...
		switch (t.cmd_action)
		{
			...
			case "get_field_list":
				...
				$('div_field_data').show();
				break;
			case "get_interim_results":
				top.frames["iframe_8"].innerHTML  = t.interim_results + "<br />-------------<br />" + t.interim_query;
				top.loadTab(8,"");		
				break;
			...
			case "save_and_load_filter":
				...
				alert("Filter saved");
				parent.loadFilter(t.item_id, 'build');
				break;
			case "save_as_filter":
		...
		<td valign="top">
			<div class="module_content">
				<form action="index.php?cmd=FilterBuilder" method="post" name="adminForm" autocomplete="off">
					...
					<input type="hidden" id="id" name="id" value="{$filter->getId()}" />
					<input type="hidden" id="filter_name_orig" name="filter_name_orig" value="{$filter->getName()}" />
					<div id="filter">
						...
						<table style="width:75%">
							<tr>
								...
								<td>
									<input type="text" id="filter_name" name="filter_name" value="{$filter->getName()}" style="width:100%" />
								</td>
							</tr>
							<tr>
								...
								<td>
									<select id="campaign_id" name="campaign_id" style="width: 90%">
										...
										{foreach name="campaign_loop" from=$campaigns item=campaign}
											<option {if $campaign->getId() == $filter->getCampaignId()}selected{/if} value="{$campaign->getId()}">{$campaign->getClientName()}</option>
										{/foreach}
									</select>
								</td>
							</tr>
							...
							<tr>
								...
								<td>
									<input type="checkbox" id="is_report_source" {if $is_report_source}checked{/if} onchange="javascript:showReportParameterDescription(this.checked);" />
									<div id="div_report_parameter_description" style="{if !$is_report_source}display:none{/if}">
										<br />
										Report Parameter Description
										<br />
										<textarea id="report_parameter_description" name="report_parameter_description" rows="4" style="width:100%;">{$report_parameter_description}</textarea>
									</div>
							...
							<tr>
								...
								<td>
									<span id="stats_company_count">{$filter->getCompanyCount()}</span>
								</td>
							</tr>
							<tr>
								<td style="width: 100px">
								...
								<td>
									<span id="stats_post_count">{$filter->getPostCount()}</span>
								</td>
							</tr>
					...
					{if $is_owner}
						...
						<input type="button" id="save_and_load" name="save_and_load" value="Save and Load" onclick="javascript:saveAndLoadFilter();"/>&nbsp;|&nbsp;
					{/if}
					<input type="button" id="save_as" name="save_as" value="Save A Copy As" onclick="javascript:saveAsFilter();"/>&nbsp;|&nbsp;
					<input type="button" id="save_as_and_load" name="save_as_and_load" value="Save A Copy As (and load)" onclick="javascript:saveAsAndLoadFilter();"/>&nbsp;|&nbsp;
					<a href="#" onclick="javascript:printFilter();" />Print <img src="{$APP_URL}app/view/images/icons/pop-up-launcher.gif" alt="Prints filter parameters in new window" title="Print filter" />
				</form>
			</div>
		</td>
