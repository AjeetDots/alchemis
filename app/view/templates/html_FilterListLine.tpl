...
<td style="text-align: center; vertical-align: middle; background-color: #F3F3F3">
	<a id="btn_display_{$filter->getId()}" title="Display current results (fast)" href="#" onclick="javascript:loadFilter({$filter->getId()}, 'reload');return false;"><img src="{$APP_URL}app/view/images/icons/table_go.png" alt="Display" title="Display currently saved results" /></a>&nbsp;
	<a id="btn_refresh_{$filter->getId()}" title="Rebuild filter using saved parameters (may take a while)" href="#" onclick="javascript:loadFilter({$filter->getId()}, 'build');return false;"><img src="{$APP_URL}app/view/images/icons/table_refresh.png" alt="Re-generate" title="Re-generate filter from database and display results" /></a>&nbsp;
