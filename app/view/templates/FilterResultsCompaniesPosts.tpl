...
iframeLocation(				parent.information, src);
		...
		function logCommunication(company_id, post_id, source_tab)

		{
			...
			top.loadTab(4,"Communication&company_id=" + company_id + "&post_id=" + post_id + "&initiative_id=" + initiative_id + "&source_tab=" + source_tab, true);
...
iframeLocation(			top.frames["iframe_7"], "index.php?cmd=WorkspaceFilter&id=" + company_id + "&post_id=" + post_id + "&initiative_id=" + top.$F("initiative_list") + "&filter_id=" + $F("filter_id"));
		...
		var last_company_class_change_id = null;

		var last_post_class_change_id = null;
	...
	<div id="div_results" class="cfg" style="border: solid 1px #ccc; padding: 2px; width: 100%; height: 704px; overflow: auto;">
		...
		<div id='div_bulk_ref' style='display: none'>
			<table style="width: 90%">
				<tr>
					<th style="width: 24%; text-align: left">Site tag</th>
		...
		<table id="table1" class="adminlist sortable" cellspacing="1">
			...
			<thead>
				...
				<tr>
					...
					<th style="width: 15%; text-align: left">Company</th>
					...
					<th style="width: 20%; text-align: left">Propensity</th>
			...
			<tbody>
				...
				{foreach name="result_loop" from=$results item=result}
					...
					<tr id="{if $result.post_id != ""}tr_post_{$result.post_id}{else}tr_{$result.id}{/if}" style="vertical-align:top">

						{*

						<td>

							{$smarty.foreach.result_loop.iteration}
						...
						*}
						...
						<td>
							...
							{if $result.job_title}
								...
								{$result.title}&nbsp;{$result.first_name}&nbsp;{$result.surname}
						...
						<td style="text-align: center; background-color: #F3F3F3" nowrap="nowrap">
							...
							<a style="cursor: pointer" id="detailsBtn_{$result.id}" title="Go to details for this post" onclick="javascript: showPost({$result.id}, {$result.post_id});return false;"><img src="{$APP_URL}app/view/images/icons/database_table.png" alt="Details" title="Details" /></a>&nbsp;

							{if $result.first_name == '' || $result.surname == ''}
							<a style="cursor: pointer" id="nonEffectivesBtn_{$result.id}" title="Log non-effective call not available as post has no contact details. Add contact details before calling"><img src="{$APP_URL}app/view/images/icons/status_offline.png" alt="Log Non-Effective" title="Log Non-Effective" /></a>
							{else}					
							<a style="cursor: pointer" id="nonEffectivesBtn_{$result.id}" title="Log a non-effective call for this post" onclick="javascript:logNonEffective('{$result.post_id}');return false;"><img src="{$APP_URL}app/view/images/icons/status_offline.png" alt="Log Non-Effective" title="Log Non-Effective" /></a>

							{/if}
							{*<a style="cursor: pointer" id="EffectivesBtn_{$result.id}" title="Log an effective call for this post" onclick="javascript:logCommunication({$result.id}, {$result.post_id}, 8);return false;"><img src="{$APP_URL}app/view/images/icons/status_offline.png" alt="Log Effective" title="Log Effective" /></a>*}
