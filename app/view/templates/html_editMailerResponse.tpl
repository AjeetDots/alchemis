...
<form id="frm_mailer_item_{$mailer_item->getId()}" name="frm_mailer_item_{$mailer_item->getId()}" action="#">
	<table class="sortable" id="sortable_edit_response">
		<tr>
			<th>Date</th>
			<td>{html_select_date 
				prefix           = "date_"
				time             = $mailer_item->getResponseDate()
				start_year       = "-1" 
				field_order      = "DMY"
		...
		<tr>	
			...
			<td>
				<table>
					...
					<tr>
						<td><input type="checkbox" id="chk_{$response.id}" name="chk_{$response.id}" {if $response.checked}checked{/if}/></td>
		...
		<tr>	
			<th>Note</th>
			<td><textarea id="note_textarea" name="note_textarea" rows="4" cols="50">{$mailer_item->getNote()}</textarea></td>
		</tr>
		<tr>
			<td colspan="2">
				<span style="float: right">
					<input type="button" value="Update" onclick="javascript:saveResponses({$mailer_item->getId()}); return false" />
