...
/**
 * Defines the app_controller_Controller class. 
...
// require_once('MDB2.php'); // Commented out - using EasySql instead
...
require_once('Auth/GeneralLogger.php');
require_once('Auth/RequestLogger.php');
...
require_once('app/base/CommandTimer.php');

/**
 * The Front Controller defines a central point of entry for every request. It processes the 
 * request and uses it to select an operation to perform.
 * @package Framework
 */
class app_controller_Controller implements app_base_Observable
{
	/**
	...
	{
		// Attach timer observer
		$this->attach(new app_base_CommandTimer());
	}

	/**
	 * Convinience method that calls init() and handleRequest() methods. It is static and the 
	 * constructor is private so the only option for client code is to kick off execution of our 
	 * system.
	 */
	public static function run()
	{
		$instance = new app_controller_Controller();

		$instance->init();
		$instance->validateSession();
		$instance->loadObjectsFromSession();
		$instance->handleRequest();
		$instance->notify();
	}
	
	/**
	 * Obtains an instance of ApplicationHelper which manages configuration data for the 
	...
	/**
	 * TODO
	 *  - tidy up once know requirements a bit further
	 * Crudely reloads possible known objests from the session so commits will works properly.
	 */
	public function loadObjectsFromSession()
	{
		require_once('app/domain/ObjectWatcher.php');
		
		// load any meetings that are in the session registry
		if (isset($_SESSION['auth_session']['communication']['meetings']))
		{
			foreach ($_SESSION['auth_session']['communication']['meetings'] as $meeting)
			{
				app_domain_ObjectWatcher::addNew($meeting);
			}
		}

		// load any info reqs that are in the session registry
		if (isset($_SESSION['auth_session']['communication']['information_requests']))
		{
			foreach ($_SESSION['auth_session']['communication']['information_requests'] as $info_req)
			{
				app_domain_ObjectWatcher::addNew($info_req);
			}
		}
		
		// load any meeting actions that are in the session registry
		if (isset($_SESSION['auth_session']['communication']['post_initiative_actions']))
		{
			...
			{
				if ($action->existsInDb($action->getId()))
				{
					app_domain_ObjectWatcher::addDirty($action);
				}
				else
				{
					app_domain_ObjectWatcher::addNew($action);
				}
			}
		}
	}
	...
	public function validateSession()
	{
		...
		$app_controller = $this->applicationHelper->appController();
		$redirect = false;
		
		try
		{
			$cmd = $app_controller->getCommand($request);
			$classname = preg_replace('/.*_/i', '', get_class($cmd));

			require_once('Auth/Session.php');
			$session = Auth_Session::singleton();
			
			// TODO - don't redirect to Login (or Logout), else get circular reference that won't resolve
// 			if ($classname != 'app_command_Login' && $classname != 'Login' && $classname != 'app_command_Logout' && $classname != 'Logout') {
			if ($classname == 'app_command_Login' || $classname == 'Login' || $classname == 'app_command_Logout' || $classname == 'Logout') {
				//echo 'In do nothing - validateSession<br />';
				// do nothing
			} else {
				//echo 'In do something - validateSession<br />';
				$redirect = $this->getRedirectValuesFromQueryString($_SERVER['QUERY_STRING']);
				if ($redirect !== false) {
				...
				{
//					echo '<p>' . __FILE__ . ':' . __LINE__ . ' User logged in</p>';
				}
				else
				{
...
//			/*
//			 * TODO - Is this the best place for RBAC checks?
//			 */
//			$session = Auth_Session::singleton();
////			$session->attach(new GeneralLogger());
//			$session->attach(new RequestLogger($cmd));
//			$session->start();
		}
		catch (app_base_AppException $e)
		{
	...
	/**
	 * Comman objects are retrieved and executed in a loop. It also gets the name of the view that 
	 * it should include.
	 */
	public function handleRequest()
	{
		...
		try
		{
			if ($debug) echo "\n\t***** handleRequest 1";
			$response = null;
			if ($cmd = $app_controller->getCommand($request))
			{
				if ($debug) echo "\n\t***** handleRequest 2";

				$classname = get_class($cmd);
				if ($debug) echo "<p>before: $classname";
				$classname = preg_replace('/.*_/i', '', get_class($cmd));
				if ($debug) echo "<p>after: $classname";

				// Log the current command
				$this->current_cmd = $classname;
				if (extension_loaded('newrelic')) {
				  newrelic_name_transaction($classname);
					$props = $request->getProperties();
					if ($props) {
						foreach ($props as $key => $prop) {
							newrelic_add_custom_parameter($key, $prop);
						}
					}
				}
			...
			if (substr($classname, 0, 4) != 'Ajax')
			{
				if($response instanceof app_controller_Response){
					...
					$this->forward($app_controller->getView($request));
				}
			}
		}
		catch (app_base_AppException $e)
	...
	/**
	 * TODO - maybe include exception handling?
	 * Ensures the correct view is pulled in.
	 * @param string $target name of the view script. Assumes path is 'app/view/' and excludes .php 
	 *        extension.
	 */
	public function forward($target)
	{
		$app_controller = $this->applicationHelper->appController();
		$view = $app_controller->resolveView($target);
		$view->execute();
	}
	 ...
	 * it should include.
	 */
	public function handleAjaxRequest(app_controller_Request $request)
	{
		// Updated by ds: suppress deprecation/notice so Ajax response is pure JSON (no PHP warnings in body).
		...
		if ($_SERVER['REQUEST_METHOD'] == 'GET' || $_SERVER['REQUEST_METHOD'] == 'POST')
		{
			...
			// Request
			$ajaxRequest = $request->getProperty('ajaxRequest');
			
			// Command
			$ajaxCommand = $request->getProperty('cmd');
			if ($debug) echo "\n\t***** Before If (2) *****\n";
			...
			else
			{
				...
				exit($json->encode($responseObject));
			}
...
//				exit(json_encode($responseObject));
			...
			if (file_exists('app/command/' . $ajaxCommand . '.php'))
			{
				require_once('app/command/' . $ajaxCommand . '.php');
				$classname = 'app_command_' . $ajaxCommand;
				
				if (class_exists($classname))
				{
					if ($debug) echo "\n\t***** created: before *****\n";
					$commandObject = new $classname($ajaxRequest);
	...
	private function getRedirectValuesFromQueryString($query_string)
	{
		// assume query string may contain 'redirect=Calendar/date/2012-10-22/client_id/1'
		// The aim of this function is to pull out any values into an array in the form
		// cmd 			=> Calendar
		// date 		=> 2012-10-01
		// client_id	=> 1
		// query_string => Calendar/date=2012-10-22/client_id=1
		//
		// This will allow us to create a useable redirect query string as required
		
		if (strpos($query_string, 'redirect=') === false) {
			return false;	
		} else {
			$tmp = strstr($query_string, 'redirect=');  // get the redirct part of the query string. NOTE: this assumes the redirect element is always last int eh querystring
			$tmp = substr($tmp, 9); // remove the 'redirect=' part of the string
			$tmp = explode('/', $tmp); // create an array

			// the cmd object will be the first element in the array
			$query_string = $tmp[0];
			$return = array();
			$return['cmd'] = $query_string;
			// take off the first element off $tmp as we've now used it
			array_shift($tmp);
			
			// the remainder of the array will be the key/value pairs we need to add to the return array
			for ($x=0; $x<=count($tmp)-1; $x = $x+2) {
				$return[$tmp[$x]] = $tmp[$x+1];
				
				// add to the return string (note: this will include '=' between key/value pairs in the returned query string)
				$query_string .= '/' . $tmp[$x] . '=' . $tmp[$x+1];
			}
...
// 			'date' 			=> '2011-10-01',
// 			'client_id'		=> '1',
// 			'query_string' 	=> 'Calendar/date=2011-10-22/client_id=1'
// 		);
	}

	public function utf8Urldecode($value)
	{
//	    print_r($value);
		if (is_array($value)) {
...
//	    	print $value;
	        $value = preg_replace_callback('/%([0-9a-f]{2})/i', function ($m) { return chr(hexdec($m[1])); }, (string) $value);
//	        print $value;
	    }
...
//       $temp[] = urldecode($decodedstr);
       $i += $num ;
   }
//   print_r($result);
//	$output="hungry but too lazy to cook.";
	$newfile="/Users/david/Sites/alchemis/newfile.txt";
