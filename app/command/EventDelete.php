...
/**
 * Defines the app_command_EventDelete class. 
...
require_once('app/mapper/EventMapper.php');
 ...
 * @package Alchemis
 */
class app_command_EventDelete extends app_command_ManipulationCommand
{
	...
	{
		$errors = array();
		$is_deleted = $request->getProperty('app_domain_Event_deleted');
		if (!$is_deleted)
		{
			$errors['app_domain_Event_deleted'] = new app_base_ValidationError("Event cannot be deleted until the 'Confirm Deletion' checkbox is ticked.");
		}
		return $errors;
	}
	...
	{
		if ($request->getProperty('app_domain_Event_deleted'))
		{
			$event = app_domain_Event::find($request->getProperty('event_id'));
			$event->markDeleted();
			$event->commit();
		}
		
		$request->setObject('app_domain_Event_due_date', $event->getDate());
//		$event = app_domain_Event::find($request->getProperty('event_id'));
		return true;
	}
...
//		$date = strftime('%Y-%m-%d', $action->getDueDate());
		$date = substr($event->getDate(), 0, 10);
