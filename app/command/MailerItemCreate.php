 ...
 * @package Alchemis
 */
class app_command_MailerItemCreate extends app_command_ManipulationCommand
{
	protected $mailer_items_added_count;
	protected $mailer_items_not_added_count;
	...
	public function doExecute(app_controller_Request $request)
	{
		...
		elseif ($task == 'save')
		{
			...
			{
				header('Location: index.php?cmd=MailerItemList&mailer_id=' . $request->getProperty('mailer_id') . '&notadded=' . $this->mailer_items_not_added_count . '&feedback=' . $this->mailer_items_added_count . ' mailer item(s) added successfully');
				exit();
			}
			else
			{
	...
	protected function processForm(app_controller_Request $request)
	{
		...
		foreach ($properties as $key => $item)
		{
			...
			if ($temp !== false)
			{
				...
				if (is_numeric($post_id))
				{
					...
					{
                        $post = app_model_Post::with('contact')->where('id', $post_id)->first();
                        ...
                        $post_initiative->setDataSourceId($data_source->id);
		...
		$this->mailer_items_not_added_count = $mailer_items_not_added_count;
		return true;
	}
