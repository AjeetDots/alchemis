 ...
 * @package Alchemis
 */
class app_command_ClientCreate extends app_command_ManipulationCommand
{
	protected $clientId;
	...
	private function processForm($request)
	{
		...
		$client->setPublishDiary(0);
		$client->commit();
		
		$this->clientId = $client->getId();
		
		// create associated campaign
		$campaign = new app_domain_Campaign();
		$campaign->setClientId($client->getId());
		$campaign->setCreatedAt(date('Y-m-d H:i:s'));
		$campaign->setCreatedBy($_SESSION['auth_session']['user']['id']);
		$campaign->commit();
		
		// create associated default initiative
		$initiative = new app_domain_Initiative();
		$initiative->setCampaignId($campaign->getId());
		$initiative->setName('Default');
		$initiative->commit();
