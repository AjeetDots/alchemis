...
/**
 * Defines the app_command_WorkspaceSearch class. 
...
require_once('app/lib/Tps.php');

/**
 * @package Alchemis
 */
class app_command_WorkspaceSearch extends app_command_Command
{
	...
	protected function makeWorkspaceCompany($request)
	{
		...
		$tpsResponse = app_lib_Tps::check($this->company->getTelephone(),$request->getProperty('refresh_number')?TRUE:FALSE);
		$request->setProperty('company_telephone_tps_status', $tpsResponse);
		// print_r($tpsResponse);die;
		...
		if (!is_null($this->post_id))
		{
            ...
            try{
                // Get TPS Response from Companies Post Phone Number
                ...
                $request->setProperty('post_telephone_tps_status', $tpsResponse);
            }catch(\Exception $e){}
		}
	}
