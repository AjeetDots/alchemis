...
class app_command_ImportCompany1_2 extends app_command_ManipulationCommand
{
    ...
    private function processForm($request) 
    {
    	...
    	$fileName = $request->getProperty('file_name');
    	
    	if ($fileName == '' or is_null($fileName)) {
    		die('No file name supplied - please restart');
    	}
    	
    	$results = $this->importCsv('data/' . $fileName);
    	
    	echo '<h2>Stage completed!</h2>';
    	echo '<p>Processed ' . $results[0]['records'] . ' records.</p>';
    	echo '<p>Added ' . $results[0]['success'] . ' records to temporary table.</p>';
    	echo '<p>Failed to add ' . $results[0]['failure'] . ' records.</p>';
    	
    	if (count($results[1]) > 0) {
    		echo '<p>The following row numbers could not be added to the temporary import table:</p><p>';
    		echo '<table>';
    	...
    	echo '<p>Please continue to next stage</p>';
    	exit();
    }
        ...
        while (($data = fgetcsv($handle, $length, $delimiter)) !== false)
        {
            if ($row > 1)
            {
                ...
                $pdata = array(
                      ...
                      'site_address_2'      => trim($data[$index++] . ' ' . $data[$index++]),
