...
/**
 * Defines the app_command_MessageDelete class. 
 ...
 * @package Alchemis
 */
class app_command_MessageDelete extends app_command_ManipulationCommand
{
	...
	{
		$errors = array();
		$is_deleted = $request->getProperty('app_domain_Message_deleted');
		if (!$is_deleted)
		{
			$errors['app_domain_Message_deleted'] = new app_base_ValidationError("Message cannot be deleted until the 'Confirm Deletion' checkbox is ticked.");
		}
		return $errors;
	}
	...
	{
		if ($request->getProperty('app_domain_Message_deleted'))
		{
			$message = app_domain_Message::find($request->getProperty('message_id'));
			$message->markDeleted();
			$message->commit();
		}
		return true;
	...
	{
		$sticky_fields = array('message_id');
	...
	{
		$message = app_domain_Message::find($request->getProperty('message_id'));
		$request->setObject('message', $message);
		$request->setObject('message_id', $request->getProperty('message_id'));
	}
