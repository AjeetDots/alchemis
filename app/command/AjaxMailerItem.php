...
/**
 * Command class to handle Ajax operations on app_domain_MailerItem objects.
 * @package alchemis
 */
class app_command_AjaxMailerItem extends app_command_AjaxCommand
{
...
//		$post_initiative_id = $this->request->item_id;
		...
		switch ($this->request->cmd_action)
		{
			case 'get_blank_response_form':
				$this->request->template = $this->getBlankResponseForm($this->request->mailer_id, $this->request->item_id);
				break;
			case 'edit_response':
				$this->request->template = $this->editResponse($this->request->mailer_id, $this->request->item_id);
				break;
			case 'save_responses':
				...
				foreach ($date_data as $date_item)
				{
					if ($date_item[1] == 'Year')
					{
						$date_working = $date_item[2];
					}
				}
				
				foreach ($date_data as $date_item)
				{
					if ($date_item[1] == 'Month')
					{
						$date_working = $date_working . '-' . $date_item[2];
					}
				}
				...
				$field_data[] = array(	'date',	$date_working);
				
				unset($date_working);
				unset($date_data);
				
//				$this->request->test = $field_data;
				$this->request->return_data = $this->addResponses($this->request->item_id, $field_data);
	...
	protected function getBlankResponseForm($mailer_id, $mailer_item_id)
	{
		$mailer = app_domain_Mailer::find($mailer_id);
		
		$responses = $mailer->findPossibleResponsesByMailerId($mailer_id);
		
		require_once('app/view/ViewHelper.php');
		$smarty = ViewHelper::getSmarty();

		$smarty->assign('mailer_id', $mailer_id);
		$smarty->assign('mailer_item_id', $mailer_item_id);		
...
//		$return_data = new stdClass();
		...
		return $smarty->fetch('html_addMailerResponse.tpl');
...
//		$possible_responses = $mailer->findPossibleResponsesByMailerId($mailer_id);
