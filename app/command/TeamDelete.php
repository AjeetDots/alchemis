...
/**
 * Defines the app_command_TeamDelete class. 
...
require_once('app/mapper/MessageMapper.php');
 ...
 * @package Alchemis
 */
class app_command_TeamDelete extends app_command_ManipulationCommand
{
	...
	{
		$errors = array();
		$is_deleted = $request->getProperty('app_domain_Team_deleted');
		if (!$is_deleted)
		{
			$errors['app_domain_Team_deleted'] = new app_base_ValidationError("Team cannot be deleted until the 'Confirm Deletion' checkbox is ticked.");
		}
		return $errors;
	}
	...
	{
		if ($request->getProperty('app_domain_Team_deleted'))
		{
			$team = app_domain_Team::find($request->getProperty('team_id'));
			$team->markDeleted();
			$team->commit();
		}
		return true;
	...
	{
		$team = app_domain_Team::find($request->getProperty('team_id'));
		$request->setObject('team', $team);
		$request->setObject('team_id', $request->getProperty('team_id'));
	}
