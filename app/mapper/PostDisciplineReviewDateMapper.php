...
/**
 * Defines the app_mapper_PostDisciplineReviewDateMapper class. 
 ...
 * @package Alchemis
 */
class app_mapper_PostDisciplineReviewDateMapper extends app_mapper_ShadowMapper implements app_domain_PostDisciplineReviewDateFinder
{
	public function __construct()
	{
		...
		// Select single
		$query = 'SELECT * from tbl_post_discipline_review_dates WHERE id = ?';
		$types = array('integer');
		$this->selectStmt = self::$DB->prepare($query, $types);
		
		// Select by post id
		$query = 'SELECT * from tbl_post_discipline_review_dates WHERE post_id = ?';
		$types = array('integer');
	...
	{
		$obj = new app_domain_PostDisciplineReviewDate($array['id']);
	...
	{
		$this->id = self::$DB->nextID('tbl_post_discipline_review_dates');
		return $this->id;
	}
	...
	{
		$query = 'INSERT INTO tbl_post_discipline_review_dates (id, post_id, discipline_id, `year_month`, communication_id, last_updated_at, last_updated_by) ' .
					'VALUES ' .
					'(?, ?, ?, ?, ?, ?, ?)';
		$types = array('integer', 'integer', 'integer', 'text', 'integer', 'date', 'integer');
		$insertStmt = self::$DB->prepare($query, $types);

		$data = array(	$object->getId(), $object->getPostId(), $object->getDisciplineId(), 
						$object->getYearMonth(), $object->getCommunicationId(), $object->getLastUpdatedAt(), $object->getLastUpdatedBy());
	...
	{
		$query = 'UPDATE tbl_post_discipline_review_dates ' .
					'SET post_id = ?, discipline_id = ?, `year_month` = ?, communication_id = ?, last_updated_at = ?, last_updated_by = ? ' .
					'WHERE id = ?';
		$types = array('integer', 'integer', 'text', 'integer', 'date', 'integer', 'integer');
	...
	/** Return app_domain_PostDisciplineReviewDate object by post_id and discipline_id
	 * @param integer $post_id
	 * @param integer $discipline_id
	 * @return app_domain_PostDisciplineReviewDate 
	 */
	public function findByPostIdAndDisciplineId($post_id, $discipline_id)
	{
		$query = 'SELECT * ' .
				'FROM tbl_post_discipline_review_dates ' .
