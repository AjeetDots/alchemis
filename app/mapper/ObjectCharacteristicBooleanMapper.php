...
/**
 * Defines the app_mapper_ObjectCharacteristicBooleanMapper class.
 ...
 * @package Alchemis
 */
class app_mapper_ObjectCharacteristicBooleanMapper extends app_mapper_ObjectCharacteristicMapper implements app_domain_ObjectCharacteristicBooleanFinder
{
	protected function init()
	{
		// Select single
		$query = 'SELECT * FROM tbl_object_characteristics_boolean WHERE id = ?';
		$this->select_stmt = self::$DB->prepare($query);

		// Select by company ID (newest first — duplicates possible)
		$query = 'SELECT id, value FROM tbl_object_characteristics_boolean WHERE characteristic_id = ? AND company_id = ? ORDER BY id DESC';
		$this->select_by_company_id_stmt = self::$DB->prepare($query);

		// Select by post ID
		$query = 'SELECT id, value FROM tbl_object_characteristics_boolean WHERE characteristic_id = ? AND post_id = ? ORDER BY id DESC';
		$this->select_by_post_id_stmt = self::$DB->prepare($query);

		// Select by post initiative ID
		$query = 'SELECT id, value FROM tbl_object_characteristics_boolean WHERE characteristic_id = ? AND post_initiative_id = ? ORDER BY id DESC';
		$this->select_by_post_initiative_id_stmt = self::$DB->prepare($query);

		// Update
		$query = 'UPDATE tbl_object_characteristics_boolean ' .
					'SET characteristic_id = ?, company_id = ?, post_id = ?, post_initiative_id = ?, value = ? ' .
					'WHERE id = ?';
		$this->update_stmt = self::$DB->prepare($query);

		// Delete
		$query = 'DELETE FROM tbl_object_characteristics_boolean WHERE id = ?';
	...
	{
		// $this->id = self::$DB->nextID('tbl_object_characteristics_boolean');
		return null;
	}
	...
	{
		$query = 'INSERT INTO tbl_object_characteristics_boolean ' .
