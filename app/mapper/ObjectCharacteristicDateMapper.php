...
class app_mapper_ObjectCharacteristicDateMapper extends app_mapper_ObjectCharacteristicMapper implements app_domain_ObjectCharacteristicDateFinder
{
	protected function init()
	{
		...
		$query = 'SELECT id, value FROM tbl_object_characteristics_date WHERE characteristic_id = ? AND company_id = ?';
		$this->select_by_company_id_stmt = self::$DB->prepare($query);
		
		// Select by post ID
		$query = 'SELECT id, value FROM tbl_object_characteristics_date WHERE characteristic_id = ? AND post_id = ?';
		$this->select_by_post_id_stmt = self::$DB->prepare($query);

		// Select by post initiative ID
		$query = 'SELECT id, value FROM tbl_object_characteristics_date WHERE characteristic_id = ? AND post_initiative_id = ?';
