...
/**
 * Defines the app_mapper_PostAgencyUserMapper class. 
 ...
 * @package Alchemis
 */
class app_mapper_PostAgencyUserMapper extends app_mapper_ShadowMapper implements app_domain_PostAgencyUserFinder
{
	public function __construct()
	{
		...
		// Select single
		$query = 'SELECT * from tbl_post_agency_users WHERE id = ?';
		$types = array('integer');
		$this->selectStmt = self::$DB->prepare($query, $types);
		
		// Select by post id
		$query = 'SELECT * from tbl_post_agency_users WHERE post_id = ?';
		$types = array('integer');
	...
	{
		$obj = new app_domain_PostAgencyUser($array['id']);
	...
	{
		$this->id = self::$DB->nextID('tbl_post_agency_users');
		return $this->id;
	}
	...
	{
		$query = 'INSERT INTO tbl_post_agency_users (id, post_id, discipline_id, type_id, communication_id, last_updated_at, last_updated_by) ' .
					'VALUES ' .
					'(?, ?, ?, ?, ?, ?, ?)';
	...
	{
		$query = 'UPDATE tbl_post_agency_users ' .
		...
		$data = array(	$object->getPostId(), $object->getDisciplineId(), 
						$object->getTypeId(),  $object->getCommunicationId(), $object->getLastUpdatedAt(), $object->getLastUpdatedBy(), $object->getId());
	...
	/** Return app_domain_PostAgencyUser object by post_id and discipline_id
	 * @param integer $post_id
	 * @param integer $discipline_id
	 * @return app_domain_PostAgencyUser 
	 */
	public function findByPostIdAndDisciplineId($post_id, $discipline_id)
	{
		$query = 'SELECT * ' .
				'FROM tbl_post_agency_users ' .
