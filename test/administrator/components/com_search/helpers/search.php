...
class SearchHelper
{
	function santiseSearchWord(&$searchword, $searchphrase)
	{
		...
		$aterms = explode( ' ', JC_String::strtolower( $searchword ) );

		// first case is single ignored word
		if ( count( $aterms ) == 1 && in_array( JC_String::strtolower( $searchword ), $search_ignore ) ) {
			$ignored = true;
		}

		// filter out search terms that are too small
		foreach( $aterms AS $aterm ) {
			if (JC_String::strlen( $aterm ) < 3) {
				$search_ignore[] = $aterm;
			}
		}
	...
	function limitSearchWord(&$searchword)
	{
		...
		if ( JC_String::strlen( $searchword ) > 20 ) {
			$searchword 	= JC_String::substr( $searchword, 0, 19 );
			$restriction 	= true;
		}

		// searchword must contain a minimum of 3 characters
		if ( $searchword && JC_String::strlen( $searchword ) < 3 ) {
			$searchword 	= '';
			$restriction 	= true;
		}
	...
	function checkNoHtml($object, $searchTerm, $fields) {
		...
		foreach($fields AS $field) {
			...
			foreach($terms AS $term) {
				if(JC_String::stristr($text, $term) !== false) {
					return true;
				}
			}
		}
	...
	{
		$textlen = JC_String::strlen($text);
		$lsearchword = JC_String::strtolower($searchword);
		$wordfound = false;
		$pos = 0;
		while ($wordfound === false && $pos < $textlen) {
			if (($wordpos = @JC_String::strpos($text, ' ', $pos + $length)) !== false) {
				$chunk_size = $wordpos - $pos;
			} else {
				$chunk_size = $length;
			}
			$chunk = JC_String::substr($text, $pos, $chunk_size);
			$wordfound = JC_String::strpos(JC_String::strtolower($chunk), $lsearchword);
			if ($wordfound === false) {
				$pos += $chunk_size + 1;
			}
		}
		...
		if ($wordfound !== false) {
			...
			if (($wordpos = @JC_String::strpos($text, ' ', $length)) !== false) {
				return JC_String::substr($text, 0, $wordpos) . '&nbsp;...';
			} else {
				return JC_String::substr($text, 0, $length);
			}
		}
	}
}
