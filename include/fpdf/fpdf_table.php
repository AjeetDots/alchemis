...
function Table_Align(){
    ...
    switch($tb_align){
        ...
        case 'XY': // use current XY position
//        	echo ('in XY');
			if (!is_null($this->table_startx)){ // $this->table_startx is set when the table position is initialised, so this is the x co-ord we need to use for all
...
function Init_Table_Position(){
	$this->table_startx = null;
    $this->table_starty = null;
