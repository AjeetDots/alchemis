...
{
// 	throw new Exception($aMinVersion);
...
//
//Check if there were any warnings (php_errormsg removed in PHP 8)
//
if (CATCH_PHPERRMSG && isset($GLOBALS['php_errormsg']) && is_string($GLOBALS['php_errormsg']) &&
...
//---------------
// CONSTRUCTOR
        function __construct(...$args) { call_user_func_array(array($this, 'JpgTimer'), $args); }
function JpgTimer() {
	$this->idx=0;
    }
...
//---------------
// CONSTRUCTOR	
        function __construct(...$args) { call_user_func_array(array($this, 'DateLocale'), $args); }
function DateLocale() {
...
class Footer {
    ...
    // PHP 8.4: PHP4-style constructors are no longer run automatically.
    // Bridge to the legacy constructor so existing code keeps working.
...
// CONSTRUCTOR
    ...
    // PHP 8.4: bridge __construct() to legacy PHP4-style constructor.
    function __construct($aWidth=300,$aHeight=200,$aCachedName="",$aTimeOut=0,$aInline=true) {
	$this->Graph($aWidth,$aHeight,$aCachedName,$aTimeOut,$aInline);
    }
//---------------
// PUBLIC METHODS	
	...
	// Now reconstruct any user URL argument (foreach for PHP 8+; each() removed)
...
// CONSTRUCTOR
	...
	$this->Text($aTxt,$aXAbsPos,$aYAbsPos);
    }
//---------------
// PUBLIC METHODS	
...
class SuperScriptText extends Text {
    ...
    private $iSimple=false;
        ...
        function __construct(...$args) { call_user_func_array(array($this, 'SuperScriptText'), $args); }
...
//---------------
// CONSTRUCTOR
        function __construct(...$args) { call_user_func_array(array($this, 'Grid'), $args); }
function Grid($aAxis) {
...
class Axis extends AxisPrototype {
        ...
        function __construct(...$args) { call_user_func_array(array($this, 'Axis'), $args); }
function Axis($img,$aScale,$color=array(0,0,0)) {
...
//---------------
// CONSTRUCTOR
        function __construct(...$args) { call_user_func_array(array($this, 'Ticks'), $args); }
...
//---------------
// CONSTRUCTOR
        function __construct(...$args) { call_user_func_array(array($this, 'LinearTicks'), $args); }
...
class RGB {
    ...
    public $img;
        ...
        function __construct(...$args) { call_user_func_array(array($this, 'RGB'), $args); }
...
class Image {
	...
	*/
	...
	if( $this->font_family > FF_FONT2+1 ) { // A TTF font so get the font file
	    ...
	    // Updated by ds: TTF::File() renamed to getFontFilePath() for PHP 7+.
	    // $this->font_file = $this->ttf->File($this->font_family,$this->font_style);
	    $this->font_file = $this->ttf->getFontFilePath($this->font_family,$this->font_style);
	}
    }
...
class RotImage extends Image {
	...
	$this->RotImage($aWidth,$aHeight,$a,$aFormat,$aSetAutoMargin);
    }
...
class ImgStreamCache {
    ...
    //---------------
    // CONSTRUCTOR
        function __construct(...$args) { call_user_func_array(array($this, 'ImgStreamCache'), $args); }
...
//---------------
// CONSTRUCTOR
        function __construct(...$args) { call_user_func_array(array($this, 'Plot'), $args); }
...
//---------------
// CONSTRUCTOR
        function __construct(...$args) { call_user_func_array(array($this, 'PlotLine'), $args); }
